...
chmod u+s /usr/local/spine/bin/spine',
    ...
    'page_title_data_source_templates' => 'Cacti - 数据源模板',
    'data_source_templates_title' => '数据源模板',
    'data_source_templates_intro' => '数据源在 Cacti 中发挥重要作用，数据源是从您在 Cacti 中监控的设备收集的数据点。',
    'data_source_templates_poller' => '数据源获取由轮询器收集的数据。',
    'data_source_templates_graph' => '然后数据源保存要呈现给图形的值。',
    'data_source_templates_association' => '然后数据源模板与图形模板关联。',
    'data_source_templates_examples' => '这些数据源可以是例如接口上的传入流量。Cacti 允许您创建数据源模板以关联到数据查询，模板允许您描述 Cacti 应该期望什么类型的数据。',
    'data_source_templates_creation' => '要创建数据源，您需要选择适当的数据输入，有几个选项，但最受欢迎的是获取 SNMP 数据或获取 SNMP 数据（索引）。',
    'data_source_templates_indexed' => '例如，您将对接口使用获取 SNMP 数据（索引），因为 Cacti 需要查询设备以找到接口索引。您将使用获取 SNMP 数据并放入单个 OID，用于不会更改的 OID，例如 CPU 使用率通常没有索引，OID 不会更改',
    'data_source_templates_types' => '您还需要告诉模板数据源类型是什么。最常见的是 Gauge 和 Counter。这些数据类型来自 RRDtool 本身。以下是支持的数据类型及其用途。有关更多信息，请参见',
    'rrd_tool_create_function' => 'RRD 工具创建函数',

    // RRD Data Types
    'gauge_title' => 'GAUGE',
    'gauge_desc' => 'GAUGE 用于温度或房间中的人数或红帽股票价值等事物。',
    'counter_title' => 'COUNTER',
    'counter_desc' => 'COUNTER 用于连续递增计数器，如路由器中的 ifInOctets 计数器。COUNTER 数据源假设计数器永远不会减少，除非计数器溢出。更新函数考虑溢出。',
    'counter_storage' => '计数器存储为每秒速率。当计数器溢出时，RRDtool 检查溢出是否发生在 32 位或 64 位边界，并通过向结果添加适当的值来相应地行动。',
    'dcounter_title' => 'DCOUNTER',
    'dcounter_desc' => 'DCOUNTER 与 COUNTER 相同，但用于表示为双精度浮点数的数量。它可以用于跟踪以非整数递增的数量，即某些例程运行所需的秒数、某些技术设备处理的总重量等。',
    'dcounter_direction' => '唯一的实质性差异是 DCOUNTER 可以是向上计数或向下计数，但不能同时进行。当前方向在第二次非未定义计数器更新时自动检测，方向的任何进一步变化都被视为重置。新方向由重置后的第二次更新确定并锁定，以及它与重置时值的差异。',
    'derive_title' => 'DERIVE',
    'derive_desc' => 'DERIVE 将存储从数据源的最后一个值到当前值的线的导数。这对于仪表很有用，例如，测量进入或离开房间的人的速率。在内部，derive 的工作方式与 COUNTER 完全相同，但没有溢出检查。因此，如果您的计数器在 32 或 64 位时不重置，您可能希望使用 DERIVE 并将其与 MIN 值 0 结合使用。',
    'dderive_title' => 'DDERIVE',
    'dderive_desc' => 'DDERIVE 与 DERIVE 相同，但用于表示为双精度浮点数的数量。',
    'counter_vs_derive_note' => '**注意**：关于 COUNTER 与 DERIVE',
    'counter_vs_derive_author' => '作者：Don Baarda don.baarda@baesystems.com',
    'counter_vs_derive_advice' => '如果您不能容忍将偶尔的计数器重置误认为合法的计数器包装，并且更喜欢对所有合法的计数器包装和重置使用"未知"，请始终使用 min=0 的 DERIVE。否则，使用具有合适最大值的 COUNTER 将为所有合法的计数器包装返回正确的值，将一些计数器重置标记为"未知"，但可能将一些计数器重置误认为合法的计数器包装。',
    'counter_vs_derive_probability' => '对于 5 分钟步长和 32 位计数器，将计数器重置误认为合法包装的概率可以说是每 1Mbps 最大带宽约 0.8%。请注意，这相当于 100Mbps 接口的 80%，因此对于高带宽接口和 32 位计数器，min=0 的 DERIVE 可能更可取。如果您使用 64 位计数器，几乎任何最大设置都将消除将重置误认为计数器包装的可能性。',
    'absolute_title' => 'ABSOLUTE',
    'absolute_desc' => 'ABSOLUTE 用于在读取时重置的计数器。这用于容易溢出的快速计数器。因此，您不是正常读取它们，而是在每次读取后重置它们，以确保在下次溢出之前有最大的可用时间。另一个用途是您计算的事物，如自上次更新以来的消息数。',
    'compute_title' => 'COMPUTE',
    'compute_desc' => 'COMPUTE 用于存储应用于 RRD 中其他数据源的公式的结果。此数据源在更新时不提供值，而是根据定义公式的 rpn 表达式从数据源的 PDP 计算其主数据点（PDP）。然后将合并函数正常应用于 COMPUTE 数据源的 PDP（即 rpn 表达式仅用于生成 PDP）。在数据库软件中，此类数据集称为"虚拟"或"计算"列。',
    'data_source_templates_variables' => '您还可以使用变量来设置数据源的名称。可用变量可以在',
    'variables_documentation' => '变量',
    'data_source_templates_variables_page' => '文档页面上看到。',
    'data_source_templates_internal_name' => '同样重要的设置是内部数据源名称。这将用于将数据源与图形模板关联。因此请确保命名为可识别的内容。',

    // Aggregate Templates page specific
    'page_title_aggregate_templates' => 'Cacti - 聚合模板',
    'aggregate_templates_title' => '聚合模板',
    'aggregate_overview_title' => '聚合概述',
    'aggregate_templates_intro' => '**聚合模板**是**图形模板**的特殊形式。它们允许您轻松创建**图形**，将来自多个**设备**的多个通用**图形**的数据组合，并允许您轻松管理生成的**聚合图形**以从其他通用**图形**添加和删除元素。',
    'aggregate_templates_creation' => '要创建通过模板管理的**聚合图形**，您首先必须创建**聚合模板**。然后从 Cacti **图形**页面，您可以选择要用作**聚合**一部分的**图形**。然后您从 Cacti 操作下拉菜单中选择 `从模板创建聚合`。',
    'aggregate_templates_behavior' => '一旦您创建了**聚合图形**，它们的行为就像任何其他 Cacti 图形一样。它们可以是树的一部分、缩放等。它们有一个额外的好处 - 您可以以非常受控的方式从聚合中添加和删除**图形**，减少在其生命周期中维护它们的工作量。',
    'aggregate_templates_changes' => '如果您希望更改由**聚合模板**管理的**图形**的设置，只需在**聚合模板**中进行更改，它们将级联到由模板管理的**聚合图形**。',
    'aggregate_template_interface_title' => '聚合模板界面',
    'aggregate_template_interface_desc' => '下面的图像显示了流量的聚合模板。',
    'aggregate_template_edit_desc' => '当您编辑**聚合模板**时，您会看到一个界面，允许您定义图形画布以及其格式。您应该实验，直到找到最适合您需求的机制。',
    'aggregate_template_prefix' => '`聚合模板` `前缀` 设置允许您提供要应用于**聚合图形**图例项目的模式。任何 `host`、`query` 或 `input` 引用都可以在 `前缀` 部分中使用，以便唯一识别**图形项目**。',
    'aggregate_graph_types' => '有几个 `图形类型` 转换处理如何在生成的**聚合图形**中处理 `AREA`、`LINEX` 和 `STACK` 项目，它们包括：',
    'keep_graph_types' => '**保持图形类型** - 不会发生转换。所有 `AREA`、`LINE` 和 `STACKS` 将保持不变。',
    'keep_type_and_stack' => '**保持类型和堆叠** - 这意味着类型将被保留，但所有数据将被堆叠，而不是简单的 LINEX，它将被转换为 LINEX:STACK',
    'convert_to_area_stack' => '**转换为区域/堆叠图形** - 所有 `LINEX` 将被转换为 `AREA` 并堆叠。',
    'convert_to_line1' => '**转换为 LINE1** - 所有**图形项目**将被转换为 `LINE1`',
    'convert_to_line2' => '**转换为 LINE2** - 所有**图形项目**将被转换为 `LINE2`',
    'convert_to_line3' => '**转换为 LINE3** - 所有**图形项目**将被转换为 `LINE3`',
    'convert_to_line1_stack' => '**转换为 LINE1/堆叠** - 所有**图形项目**将被转换为 `LINE1` 并堆叠。',
    'convert_to_line2_stack' => '**转换为 LINE2/堆叠** - 所有**图形项目**将被转换为 `LINE2` 并堆叠。',
    'convert_to_line3_stack' => '**转换为 LINE3/堆叠** - 所有**图形项目**将被转换为 `LINE3` 并堆叠。',
    'totaling_setting' => '`总计` 设置有多个值。它们包括：',
    'no_totals' => '**无总计** - 图例中不会有数据总和',
    'print_all_legend_items' => '**打印所有图例项目** - 意味着所选的所有图例项目都将包含总计',
    'print_totaling_legend_items' => '**仅打印总计图例项目** - 此选项意味着图例将把所有图例项目总计为单个图例。',
    'total_type_setting' => '`总计类型` - 将在**图形**上创建通用元素的分组，并在通用元素发生变化时重置堆叠规则。选项包括：',
    'total_similar_data_sources' => '**总计相似数据源** - 意味着您将通过相似的**数据源**名称总计图例项目和**数据源**，例如 `traffic_out` 和 `traffic_in`。',
    'total_all_data_sources' => '**总计所有数据源** - 意味着您将对所有**数据源**的值求和，无论其**数据源**名称如何。',
    'prefix_gprint_totals' => '使用 `总计类型` 时，您可以选择使用 `GPRINT 总计前缀` 以文本值额外为您的图例加上前缀。默认值在大多数情况下都有效。',
    'reorder_type_setting' => '`重新排序类型` 将在图形上重新排序**数据源**在其各自分组内，以便它们以通用方式排序，按字母顺序。选项包括：',
    'no_reordering' => '**无重新排序** - 不对排序进行任何更改。',
    'data_source_graph_order' => '**数据源，图形** - 按**数据源**名称排序，然后按**图形**名称',
    'graph_data_source_order' => '**图形，数据源** - 按**图形**名称排序，然后按**数据源**名称',
    'base_graph_order' => '**基本图形顺序** - 仅关注**图形**名称',
    'graph_template_items_section' => '`图形模板项目` 部分允许您在生成的**聚合图形**中跳过或总计（即包含）**图形项目**。当您考虑生成的**聚合**图形的外观时，有些元素根本不会产生好看的**图形**。在这些情况下，您将希望从生成的**聚合图形**中删除它们。',
    'color_template_option' => '`颜色模板` 选项允许您在显示生成的**聚合图形**上的元素时使用不同的颜色旋转。**颜色模板**可以从 `控制台 > 模板` 下的**颜色模板**菜单选择中添加和删除。',
    'aggregate_override_sections' => '几个部分允许您覆盖生成的**聚合图形**中任何通用**图形模板**元素。我们不会在这里解释这些选项，只是让您知道您可以在生成的**聚合图形**中覆盖它们。',
    'aggregate_summary_title' => '摘要',
    'aggregate_summary_desc' => '在使用**聚合模板**时，您可以使用几种选项组合。其中一些选项会导致可怕和意外的结果，因此您必须实验，直到您想出一个理想的**聚合模板**。',

    // Color Templates page specific
    'page_title_color_templates' => 'Cacti - 颜色模板',
    'color_templates_title' => '颜色模板',
    'color_templates_intro' => '**颜色模板**定义要用于 Cacti 中**聚合图形**的颜色列表。当您向**聚合图形**添加**图形**时，您需要在**聚合图形**内区分一个**图形**与下一个。这些**颜色模板**是将以轮询方式循环使用以渲染**聚合图形**的颜色列表。',
    'color_templates_example' => '因此，例如，如果您的**颜色模板**使用 8 种不同的**颜色**，而您的**聚合图形**包含 16 个*图形项目*，那么每种颜色将在**聚合图形**中使用两次。',
    'color_templates_standard' => '下面，您可以看到四个标准**颜色模板**，您可以看到您有能力*删除*或*复制***颜色模板**。与其他 Cacti 对象一样，您将不被允许*删除*正在使用的**颜色模板**。',
    'color_templates_edit_screen' => '在下面的图像中，您可以看到**颜色模板**编辑屏幕。这个简单的屏幕允许您在列表中添加、删除和重新排序颜色。',
    'color_templates_cacti_colors' => '如下图所示，只允许选择 Cacti **颜色**用于聚合**颜色模板**。如果您希望搜索大约 340 个传统和*命名颜色*的列表，可以在**颜色**下拉菜单中输入。',

    // Common template terms
    'data_source_template' => '数据源模板',
    'data_source_templates' => '数据源模板',
    'aggregate_template' => '聚合模板',
    'aggregate_templates' => '聚合模板',
    'aggregate_graphs' => '聚合图形',
    'color_template' => '颜色模板',
    'color_templates' => '颜色模板',
    'graph_items' => '图形项目',
    'data_points' => '数据点',
    'primary_data_points' => '主数据点',
    'rpn_expression' => 'RPN 表达式',
    'consolidation_functions' => '合并函数',
    'virtual_columns' => '虚拟列',
    'computed_columns' => '计算列',
    'internal_data_source_name' => '内部数据源名称',
    'round_robin_fashion' => '轮询方式',
    'named_colors' => '命名颜色',
    'legacy_colors' => '传统颜色',

    // Automation Networks page specific
    'page_title_automation_networks' => 'Cacti - 自动化网络',
    'automation_networks_title' => '自动化网络',
    'automation_networks_intro' => '本节将描述 Cacti 中的**自动化网络**。',
    'automation_networks_adding' => '在自动化插件中添加要扫描的网络很容易。在主控制台上点击自动化。进入下面的页面后，点击页面右上角的 +。',
    'automation_networks_main_desc' => '自动化网络',
    'automation_networks_subnet_desc' => '您现在将看到下面的页面。如果您想扫描 192.168.1.0/24，您将在子网范围文本框中输入该内容，然后以 CIDR 格式输入子网。',
    'automation_networks_options' => '其他重要选项包括',

    // Automation Networks options table
    'automation_option_field' => '选项',
    'automation_description_field' => '描述',
    'schedule_type_option' => '计划类型',
    'schedule_type_desc' => '您希望多久扫描一次此子网以查找设备',
    'discovery_threads_option' => '发现线程',
    'discovery_threads_desc' => '扫描期间要生成多少个进程',
    'max_runtime_option' => '最大运行时间',
    'max_runtime_desc' => '防止扫描无限期运行',
    'auto_add_option' => '自动添加到 Cacti',
    'auto_add_desc' => '如果设备可通过 SNMP 访问并匹配来自此子网的规则，则将添加该设备',
    'netbios_option' => 'Netbios',
    'netbios_desc' => '扫描 netbios 名称',
    'ping_method_option' => 'Ping 方法',
    'ping_method_desc' => '要发送的 ping 数据包类型',
    'ping_port_option' => 'Ping 端口',
    'ping_port_desc' => '尝试连接的 TCP/UDP 端口',
    'ping_timeout_option' => 'Ping 超时',
    'ping_timeout_desc' => '用于主机 ICMP 和 UDP ping 的超时值。此主机 SNMP 超时值适用于 SNMP ping',
    'ping_retries_option' => 'Ping 重试',
    'ping_retries_desc' => '初始失败后，Cacti 在失败前将尝试的 ping 重试次数',
    'automation_networks_edit_desc' => '自动化网络编辑',

    // Discovered Devices page specific
    'page_title_discovered_devices' => 'Cacti - 自动化发现设备',
    'discovered_devices_title' => '自动化发现设备',
    'discovered_devices_intro' => '从自动化启动扫描后，Cacti 将比较找到的设备与您设置的设备规则和 SNMP 选项。',
    'discovered_devices_no_match' => '如果 Cacti 无法将设备匹配到设备模板或无法找到适当的 SNMP 凭据。',
    'discovered_devices_auto_add_off' => '另外，如果您将自动添加到 Cacti 设置为关闭，设备也将被放在此部分中。',
    'discovered_devices_navigation' => '可以通过导航到以下区域找到发现设备部分。',
    'discovered_devices_dropdown_desc' => '发现设备',
    'discovered_devices_scan_results' => '下面您可以看到在扫描期间找到的不符合匹配条件的设备。',
    'discovered_devices_ip_hostname' => '您将看到被扫描的 IP 以及如果可用的话通过 DNS 或 netbios 解析的主机名。',
    'discovered_devices_list_desc' => '发现设备',
    'discovered_devices_selection' => '然后您可以选择您有兴趣添加的设备，点击设备旁边的复选框',
    'discovered_devices_add_menu' => '在下拉菜单中选择添加设备，这将弹出下面的菜单。从此菜单您将能够填写新设备的详细信息。',
    'discovered_devices_add_menu_desc' => '发现设备',

    // Device Rules page specific
    'page_title_device_rules' => 'Cacti - 设备规则',
    'device_rules_title' => '设备规则',
    'device_rules_intro' => '设备规则允许您在网络发现过程中自动为设备分配设备模板。',
    'device_rules_creation' => '要创建设备规则，您必须首先创建设备模板。然后，从设备规则页面，您可以创建基于设备的 SNMP 系统描述或其 SNMP 系统对象 ID 匹配设备的规则。',
    'device_rules_matching' => '当发现设备时，Cacti 将尝试将设备与设备规则匹配。如果找到匹配项，与设备规则关联的设备模板将应用于设备。',
    'device_rules_priority' => '设备规则按其优先级顺序处理。第一个匹配的规则将应用于设备。',

    // Graph Rules page specific
    'page_title_graph_rules' => 'Cacti - 图形规则',
    'graph_rules_title' => '图形规则',
    'graph_rules_intro' => '图形规则允许您在网络发现过程中自动为设备创建图形。',
    'graph_rules_creation' => '要创建图形规则，您必须首先创建图形模板和数据查询。然后，从图形规则页面，您可以创建基于各种条件自动创建图形的规则。',
    'graph_rules_matching' => '当发现设备并匹配设备规则时，Cacti 将处理任何图形规则以自动为设备创建图形。',
    'graph_rules_types' => '有几种类型的图形规则，包括图形模板规则和基于数据查询的图形规则。',

    // Tree Rules page specific
    'page_title_tree_rules' => 'Cacti - 树规则',
    'tree_rules_title' => '树规则',
    'tree_rules_intro' => '树规则允许您在网络发现过程中自动将设备和图形放置到树中。',
    'tree_rules_creation' => '要创建树规则，您必须首先创建树。然后，从树规则页面，您可以创建自动将设备和图形放置到适当树位置的规则。',
    'tree_rules_matching' => '当发现设备时，Cacti 将处理树规则以自动将设备及其图形组织到树结构中。',
    'tree_rules_hierarchy' => '树规则可以基于设备属性（如站点、设备类型或自定义字段）创建分层结构。',

    // SNMP Options page specific
    'page_title_snmp_options' => 'Cacti - SNMP 选项',
    'snmp_options_title' => 'SNMP 选项',
    'snmp_options_intro' => 'SNMP 选项定义在网络发现过程中将使用的 SNMP 凭据和设置。',
    'snmp_options_creation' => '要创建 SNMP 选项，您需要定义 SNMP 版本、团体字符串、用户名、密码和其他与 SNMP 相关的设置。',
    'snmp_options_discovery' => '在网络发现期间，Cacti 将尝试使用定义的 SNMP 选项连接到设备，以确定设备是否启用了 SNMP。',
    'snmp_options_versions' => 'SNMP 选项支持 SNMP 版本 1、2c 和 3，每个版本都有自己的特定配置要求。',
    'snmp_options_security' => '对于 SNMP v3，您可以配置身份验证和隐私协议以进行安全的 SNMP 通信。',

    // Common automation terms
    'automation_networks' => '自动化网络',
    'discovered_devices' => '发现设备',
    'device_rules' => '设备规则',
    'graph_rules' => '图形规则',
    'tree_rules' => '树规则',
    'snmp_options' => 'SNMP 选项',
    'network_discovery' => '网络发现',
    'device_template_assignment' => '设备模板分配',
    'automatic_graph_creation' => '自动图形创建',
    'tree_organization' => '树组织',
    'snmp_credentials' => 'SNMP 凭据',
    'discovery_process' => '发现过程',
    'automation_rules' => '自动化规则',
    'subnet_scanning' => '子网扫描',
    'device_matching' => '设备匹配',
    'rule_priority' => '规则优先级',
    'snmp_versions' => 'SNMP 版本',
    'community_strings' => '团体字符串',
    'authentication_protocols' => '身份验证协议',
    'privacy_protocols' => '隐私协议',

    // Networks page specific
    'page_title_networks' => 'Cacti - 网络',
    'networks_title' => '网络',
    'networks_intro' => 'Cacti 中的**网络**部分提供全面的网络管理和监控功能。本节涵盖网络发现、设备管理和自动化功能的各个方面，帮助您高效监控网络基础设施。',
    'networks_overview' => 'Cacti 的网络管理功能包括自动网络发现、用于自动设备分类的设备规则、用于自动图形创建的图形规则，以及用于组织网络拓扑的树规则。',
    'networks_automation_title' => '网络自动化',
    'networks_automation_desc' => 'Cacti 提供强大的自动化功能，可以自动发现网络上的设备，根据预定义规则对其进行分类，并创建适当的图形并在树中组织它们。',
    'networks_discovery_title' => '网络发现',
    'networks_discovery_desc' => '网络发现过程允许 Cacti 自动扫描网络范围并识别可以监控的设备。此过程使用 SNMP 和其他协议来检测和分类网络设备。',
    'networks_device_management_title' => '设备管理',
    'networks_device_management_desc' => '一旦发现设备，Cacti 可以自动应用设备模板，创建适当的图形，并根据您的网络拓扑和管理要求组织设备。',
    'networks_monitoring_title' => '网络监控',
    'networks_monitoring_desc' => 'Cacti 提供全面的网络监控功能，包括接口统计、设备性能指标，以及通过 SNMP、脚本和其他数据收集方法进行的自定义监控。',
    'networks_organization_title' => '网络组织',
    'networks_organization_desc' => '使用 Cacti 的树和站点管理功能，以符合您的网络拓扑和组织结构的逻辑层次结构组织您的网络设备和图形。',
    'networks_features_title' => '主要网络功能',
    'networks_feature_automation' => '**自动化网络** - 配置网络范围以进行自动发现',
    'networks_feature_discovered' => '**发现设备** - 管理网络扫描期间找到的设备',
    'networks_feature_device_rules' => '**设备规则** - 定义自动设备模板分配规则',
    'networks_feature_graph_rules' => '**图形规则** - 配置自动图形创建规则',
    'networks_feature_tree_rules' => '**树规则** - 设置自动设备和图形组织',
    'networks_feature_snmp' => '**SNMP 选项** - 配置用于设备通信的 SNMP 凭据和设置',
    'networks_getting_started_title' => '网络管理入门',
    'networks_getting_started_desc' => '要开始使用 Cacti 的网络管理功能，首先配置您的 SNMP 选项，然后设置自动化网络以定义您要监控的网络范围。创建设备规则以自动分类发现的设备，并配置图形和树规则以自动组织您的监控数据。',
    'networks_best_practices_title' => '最佳实践',
    'networks_best_practices_desc' => '为了实现最佳网络管理，按站点和功能组织您的设备，使用一致的命名约定，为不同设备类型配置适当的 SNMP 凭据，并定期审查和更新您的自动化规则，以确保它们与您不断发展的网络基础设施相匹配。',

    // Data Profiles page specific
    'page_title_data_profiles' => 'Cacti - 数据源配置文件',
    'data_profiles_title' => '数据源配置文件',
    'data_profiles_intro' => '本节将描述 Cacti 中的**数据源配置文件**。',
    'data_profiles_disk_usage' => '以下值是相应数据配置文件的每个数据源的磁盘使用量',
    'data_profiles_polling_time' => '轮询时间',
    'data_profiles_type' => '类型',
    'data_profiles_size_per_ds' => '每个数据源大小',
    'data_profiles_30_second' => '30 秒',
    'data_profiles_daily' => '每日',
    'data_profiles_weekly' => '每周',
    ...
    'cdefs_background_desc' => 'Cacti 中的 CDEF 与 RRDtool 中的 CDEF 是一对一的类比。Cacti 只是提供了一个创建和管理它们的界面。一旦在 Cacti 中创建了 CDEF，它们就可以全局导入和导出。',
    'cdefs_mathematical_formulas' => 'CDEF 是数学公式，可以修改来自一个到多个数据源或您在**图形模板**中拥有的 VNAME 的数字数据。',
    'cdefs_rpn_format' => '数学公式的格式称为逆波兰表示法（RPN）。RPN 是工程师将方程式输入早期 HP 和其他计算器以解决工程问题的早期形式。我们今天仍然使用它的原因是它遵循简单的堆栈原理。换句话说，它没有损坏。',
    'cdefs_complexity' => 'CDEF 可能变得非常复杂，因为 RRDtool 命令集中有几个数学函数可用。',
    'cdefs_interface_title' => 'CDEF 界面',
    'cdefs_interface_desc' => '在下面的图像中，您可以看到默认情况下 Cacti 中包含的所有 CDEF。有相当多的。使用中的许多公式都相当简单。如果您想查看有关如何使用 CDEF 的教程，您应该转到 RRDtool 教程。RRDtool 网站上也有文档。',
    'cdefs_delete_duplicate' => '在此图像中，您可以看到您有能力删除或复制 CDEF，但请注意，您将无法删除与 Cacti **图形**或**图形模板**关联的任何 CDEF。',
    'cdefs_edit_screen' => '当您点击 CDEF 的名称时，您将进入编辑屏幕。从那里您将看到堆栈的有序列表。它通常以 CURRENT_DATA_SOURCE 之类的内容开始，这意味着当您向**图形模板**或**图形**添加**图形项目**时，您可以选择 CDEF。与该**图形项目**关联的**数据源**是 CURRENT_DATA_SOURCE。',
    'cdefs_stack_operations' => '之后，您可能会看到一个数字，后跟一个数学运算符。这是 CDEF 的最简单形式。如果您启用了拖放功能，您可以使用拖放重新排序 CDEF 项目。否则，您将看到允许您上下移动 CDEF 项目的箭头。',
    'cdefs_data_types' => '编辑 CDEF 时，第一个决定是您要在堆栈上放置什么类型的数据，如下图所示的选项。它们包括：',
    'cdefs_type_function' => '函数',
    'cdefs_type_function_desc' => '我们将在下面描述的数学函数',
    'cdefs_type_operator' => '运算符',
    'cdefs_type_operator_desc' => '常见的数学运算符，包括（+、-、*、/ 和 %）',
    'cdefs_type_another_cdef' => '另一个 CDEF',
    'cdefs_type_another_cdef_desc' => '您可以在您的 CDEF 中引用另一个 CDEF',
    'cdefs_type_special_data_source' => '特殊数据源',
    'cdefs_type_special_data_source_desc' => '这些是 RRDtool 提供的特殊变量',
    'cdefs_type_custom_string' => '自定义字符串',
    'cdefs_type_custom_string_desc' => '您可以输入任何自定义字符串或数字',

    // VDEFs page specific
    'page_title_vdefs' => 'Cacti - VDEF',
    'vdefs_title' => 'VDEF',
    'vdefs_intro' => 'Cacti 中的 VDEF 用于定义可以对现有数据源执行统计操作的虚拟数据源。',
    'vdefs_description' => 'VDEF 允许您创建虚拟数据源，可以计算数据的统计信息，如平均值、最大值、最小值、总计和其他数学运算。',
    'vdefs_usage' => 'VDEF 通常在图形模板中使用，以在常规数据源旁边显示统计信息。',

    // Colors page specific
    'page_title_colors' => 'Cacti - 颜色',
    'colors_title' => '颜色',
    'colors_intro' => '本节将描述 Cacti 中的**颜色**。',
    'colors_description' => 'Cacti 中的颜色用于定义图形的视觉外观。您可以创建自定义颜色或使用预定义的调色板。',
    'colors_management' => '颜色管理界面允许您添加、编辑和删除可在整个 Cacti 中用于图形渲染的颜色。',
    'colors_hex_values' => '颜色使用十六进制颜色值定义（例如，#FF0000 表示红色，#00FF00 表示绿色，#0000FF 表示蓝色）。',

    // GPRINTs page specific
    'page_title_gprints' => 'Cacti - GPRINT',
    'gprints_title' => 'GPRINT',
    'gprints_intro' => '本节将描述 Cacti 中的 **GPRINT**。',
    'gprints_description' => 'GPRINT（图形打印）用于在图形上显示统计信息。它们定义数值的格式化和显示方式。',
    'gprints_formatting' => 'GPRINT 控制图形上显示的数字格式，包括小数位、单位和文本格式。',
    'gprints_presets' => 'Cacti 包含几个预定义的 GPRINT 预设，用于常见的格式需求，您可以为特定要求创建自定义 GPRINT。',
    'gprints_usage' => 'GPRINT 通常在图形模板中使用，以显示数据源的当前值、平均值、最大值和最小值。',

    // Common preset terms
    'data_profiles' => '数据配置文件',
    'data_source_profiles' => '数据源配置文件',
    'cdefs' => 'CDEF',
    'vdefs' => 'VDEF',
    'colors' => '颜色',
    'gprints' => 'GPRINT',
    'gprint_presets' => 'GPRINT 预设',
    'mathematical_formulas' => '数学公式',
    'reverse_polish_notation' => '逆波兰表示法',
    'rpn' => 'RPN',
    'stack_operations' => '堆栈操作',
    'data_source_statistics' => '数据源统计',
    'graph_formatting' => '图形格式',
    'color_palette' => '调色板',
    'hexadecimal_values' => '十六进制值',
    'statistical_operations' => '统计操作',
    'virtual_data_sources' => '虚拟数据源',
    'numerical_formatting' => '数值格式',
    'preset_management' => '预设管理',
    'graph_rendering' => '图形渲染',
    'data_visualization' => '数据可视化',

    // Import Template page specific
    'page_title_import_template' => 'Cacti - 模板导入',
    'import_template_title' => '模板导入',
    'import_template_intro' => '本节将描述 Cacti 中的**模板导入**。',
    'import_template_description' => '模板导入文件是 XML 文件，用于添加对更多图形类型和不同设备的支持，可以与他人共享。这些模板文件可以是自定义制作的，也可以从 cacti 论坛的社区下载。',
    'import_template_process' => '下载所需的图形/设备模板后，您需要将模板导入到 cacti 系统中。要导入设备/图形模板，请转到 `控制台 > 导入/导出 > 导入模板`，您现在将看到下面的页面',
    'import_template_important_note' => '**重要说明**：导入模板时，请确保将数据源配置文件与您首选的**数据源配置文件**匹配。',
    'import_template_select_file' => '首先，您必须点击**选择文件**并浏览到 XML 文件在您计算机上的位置。Cacti 将默认预览导入以检查任何问题。如果没有发现问题，您可以选择相同的文件，然后取消选中预览选项进行导入，之后模板应该可用。',
    'import_template_remove_orphans' => '`删除孤立项`选项只应在您的模板已损坏且纠正它们的最佳方法是重新开始时使用。此选项将从任何**图形**中删除不出现在要导入的**图形模板**中的任何**图形项目**。请谨慎使用此选项。',

    // Export Template page specific
    'page_title_export_template' => 'Cacti - 模板导出',
    'export_template_title' => '模板导出',
    'export_template_intro' => 'Cacti 允许以 XML 格式导出**设备**、**图形**和**数据**模板。您可以通过转到 `控制台 > 导入/导出 > 模板导出` 来访问下面的屏幕。当您到达此页面时，最受欢迎的选项是导出**设备模板**，但决定权在用户。选择对象类型后，将显示可用模板列表。最受欢迎的选项是保存到文件，但您还有其他选项。',
    'export_template_guid' => '在模板内部，Cacti 使用**哈希**全局唯一 ID (GUID) 来唯一标识每个模板组件。因此，您导出的任何模板都可以与全球任何人共享而不会产生歧义。请参阅下面的 Linux 设备模板示例。',
    'export_template_components' => '在此模板中有许多组件：',
    'export_template_component_1' => '设备模板名称本身及其 GUID',
    'export_template_component_2' => '所有图形模板及其 GUID',
    'export_template_component_3' => '所有图形模板输入及其 GUID',
    'export_template_component_4' => '所有数据模板及其 GUID',
    'export_template_component_5' => '所有数据输入方法及其 GUID',
    'export_template_component_6' => '所有 GPrint 预设及其 GUID',
    'export_template_component_7' => '所有 CDEF 和 VDEF 及其 GUID',
    'export_template_component_8' => '所有数据源配置文件及其 GUID',
    'export_template_encoding' => '虽然输出是人类可读的，但某些列被编码以免对 XML 输出格式造成问题。要读取它们，您必须在 PHP 或其他语言中对它们进行 base64_decode。',
    'export_template_format' => '目前它们以 XML 格式保存，但正在讨论使这些模板采用 YAML 格式。',

    // Common template import/export terms
    'template_import' => '模板导入',
    'template_export' => '模板导出',
    'template_exporting' => '模板导出',
    'xml_files' => 'XML 文件',
    'template_files' => '模板文件',
    'graph_types' => '图形类型',
    'device_templates' => '设备模板',
    ...
    'select_file' => '选择文件',
    'preview_import' => '预览导入',
    'remove_orphans' => '删除孤立项',
    'global_unique_id' => '全局唯一 ID',
    'guid' => 'GUID',
    'hash_identifier' => '哈希标识符',
    'template_components' => '模板组件',
    'xml_format' => 'XML 格式',
    'yaml_format' => 'YAML 格式',
    'base64_decode' => 'Base64 解码',
    'human_readable' => '人类可读',
    'template_sharing' => '模板共享',
    'community_templates' => '社区模板',
    'custom_templates' => '自定义模板',
    'damaged_templates' => '损坏的模板'
);
?>
