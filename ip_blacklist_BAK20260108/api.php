...
function generateRiskAnalysis($ip, $isBlacklisted, $geoData) {
    ...
    if (count($countryConsensus) > 1) {
        $riskScore += 5;
        $riskFactors[] = 'Inconsistent country data across providers / 各提供者的國家數據不一致';
    }
...
function batchQuery($ips) {
    ...
    $results = []; $blacklistedCount = 0; $safeCount = 0;
    foreach (array_slice($ips, 0, 100) as $ip) {
        $ip = trim($ip); if (empty($ip)) continue;
        if (filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
            $isBlacklisted = isBlacklisted($ip);
            $results[] = ['ip' => $ip, 'blacklisted' => $isBlacklisted, 'status' => $isBlacklisted ? 'blocked' : 'safe'];
            $isBlacklisted ? $blacklistedCount++ : $safeCount++;
        } else { $results[] = ['ip' => $ip, 'error' => 'Invalid IP']; }
    }
    return ['type' => 'batch_query', 'total' => count($results), 'blacklisted' => $blacklistedCount,
        'safe' => $safeCount, 'results' => $results, 'timestamp' => date('Y-m-d H:i:s')];
}
