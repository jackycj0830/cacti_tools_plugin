...
switch ($action) {
    ...
    case 'local_search':
        ...
        break;
    case 'local_batch':
        echo json_encode(queryLocalBatch(json_decode($_POST['ips'] ?? '[]', true)));
        break;
    case 'archive_stats':
        echo json_encode(getArchiveStatistics());
        break;
    case 'archive_stats_detailed':
        echo json_encode(getArchiveStatisticsDetailed());
        break;
    case 'archive_countries':
        echo json_encode(getArchiveCountries());
        break;
    case 'country_ip_ranges':
...
function queryLocalBatch($ips) {
    ...
    $cache = getIPCache();
    if (!$cache) {
        return ['error' => 'Database not available / 資料庫不可用'];
    }
    ...
    // Limit to 100 IPs for local batch query
    $ipsToProcess = array_slice($ips, 0, 100);
    ...
    foreach ($ipsToProcess as $ip) {
        ...
        // Query from local archive database
        // Note: queryLocal() returns data through archiveRowToResult() with nested structure
        $archived = $cache->queryLocal($ip);

        if ($archived) {
            $foundCount++;
            // archiveRowToResult returns 'blacklisted' directly
            $isBlacklisted = (bool)($archived['blacklisted'] ?? false);
            $isBlacklisted ? $blacklistedCount++ : $safeCount++;
            ...
            // Extract geo info from nested geo object
            $geo = $archived['geo'] ?? [];
            $riskAnalysis = $archived['riskAnalysis'] ?? [];

            // Extract VT data from archived record
            $vtFlagged = $archived['vtDetectionFlagged'] ?? null;
            $vtTotal = $archived['vtDetectionTotal'] ?? null;
            $vtLink = $archived['vtLink'] ?? null;
            $vtQueriedAt = $archived['vtQueriedAt'] ?? null;
            $vtMalicious = $archived['vtMalicious'] ?? null;
            $vtSuspicious = $archived['vtSuspicious'] ?? null;

            // Determine VT status from detection counts
            $vtStatus = null;
            if ($vtFlagged !== null) {
                $vtStatus = ($vtFlagged > 0) ? 'malicious' : 'safe';
            }
            ...
            $results[] = [
                ...
                'isp' => $geo['isp'] ?? '-',
                'org' => $geo['org'] ?? '-',
                'riskScore' => (int)($riskAnalysis['riskScore'] ?? 0),
                'riskLevel' => $riskLevel,
                'threatInfo' => $threatInfo,
    ...
    return [
        ...
        'timestamp' => date('Y-m-d H:i:s'),
        'source' => 'archive',
        'note' => count($ipsToProcess) < count($ips)
            ? '已限制為前100個IP / Limited to first 100 IPs'
            : null
    ];
}
...
function getArchiveStatisticsDetailed() {
    ...
    $stats = $cache->getArchiveStatsDetailed();
...
/**
 * Get blacklisted IPs grouped by country with IP ranges
 * 獲取按國家分組的黑名單IP及其IP範圍
...
function getCountryIPRanges($limit = 10) {
    ...
    $limit = min(max(1, (int)$limit), 20); // Limit between 1 and 20
    $data = $cache->getCountryIPRanges($limit);
