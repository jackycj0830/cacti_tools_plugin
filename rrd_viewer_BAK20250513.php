...
if (isset($_GET['file'])) {
    ...
    if ($return_var === 0) {
        ...
        foreach ($output as $line) {
            // 將第一行視為標題
            if (empty($columns) && preg_match('/^\s*\w+/', $line)) {
                $columns = preg_split('/\s+/', trim($line)); // 提取列標題
                continue;
            }
            ...
            if (preg_match('/^\d+:/', $line)) {
                ...
                $dataValues = array_slice($parts, 1); // 提取數據值

                // 去除數值中的 (e+後面所有數字)
                $cleanValues = array_map(function ($value) {
                    return preg_replace('/e\+\d+/', '', $value);
                }, $dataValues);

                $rows[] = array_merge([$readableTime], $cleanValues); // 合併時間與數據
            }
        }
        ...
        if (!empty($columns)) {
            echo "<table>";
            echo "<tr><th>時間</th><th>" . implode("</th><th>", $columns) . "</th></tr>";
